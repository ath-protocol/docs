...
graph TB
    subgraph "Agent Proves Identity"
        Key["🔐 Private Key"] -->|"signs"| JWT["Attestation JWT"]
        Doc["📄 agent.json"] -->|"publishes"| PubKey["🔓 Public Key"]
    end
    
    subgraph "Two-Gate Authorization"
        JWT -->|"verified by"| Server["🏢 Server"]
        PubKey -->|"fetched by"| Server
        Server -->|"Phase A"| Approval["✅ Service Approves"]
        OAuth["👤 User Consent"] -->|"Phase B"| Consent["✅ User Consents"]
    end
    
    subgraph "Scoped Access"
        Approval --> Intersection["⚖️ Scope Intersection"]
        Consent --> Intersection
        Intersection --> Token["🎫 Bound ATH Token"]

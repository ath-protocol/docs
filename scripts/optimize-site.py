...
Adapted from https://github.com/ath-protocol/agent-trust-handshake-protocol/blob/main/scripts/optimize-site.py

Performs optimizations on the exported _site/ directory:

1. Path rewriting: rewrites root-relative paths when BASE_PATH is set
   (for GitHub Pages subpath hosting, e.g. /docs/).  This includes:
...
2. Prefetch: injects <link rel="prefetch"> for sibling doc pages so sidebar
   navigation feels instant after the first page load.

3. Preload: injects <link rel="preload"> for critical CSS and font files.

4. Service Worker: writes sw.js and registers it from every page for
   cache-first offline support.

Supports both deployments:
  - Root: doc.ath-protocol.io (BASE_PATH="" → no rewriting)
  - Subpath: ath-protocol.github.io/docs (BASE_PATH="/docs" → rewrite all paths)
...
def rewrite_html_paths(html: str, base: str) -> str:
    ...
    for part in parts:
        if part.startswith("<script"):
            ...
            for prefix in (
                "_next/", "logo/", "favicons/", "favicon", "foundations/",
                "server/", "gateway/", "client/", "appendices/",
                "sitemap", ".well-known/", "sw.js",
            ):
...
def rewrite_js_paths(js: str, base: str) -> str:
    ...
    for prefix in (
        "foundations/", "server/", "gateway/", "client/", "appendices/",
    ):
...
def patch_nextjs_basepath(js: str, base: str) -> str:
    """Patch Next.js hasBasePath / removeBasePath / addBasePath modules."""
    js = re.sub(
...
def rewrite_paths(site_dir: Path, base: str) -> int:
    ...
    next_dir = site_dir / "_next"
    if next_dir.exists():
        for js_path in next_dir.rglob("*.js"):
        ...
        for css_path in next_dir.rglob("*.css"):
